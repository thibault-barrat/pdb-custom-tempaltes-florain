<?php
/**
 * custom template for a responsive list layout using CSS3 flexbox and with an additional filter
 * 
 * @version 0.3
 */
    ...
    <div class="pdb-searchform">
      ...
      <?php if ( $filter_mode == 'filter' || $filter_mode == 'both' ) : ?>
        ...
        <div class="control-group">
          <label class="control-label">Filtrer par catégorie d'acteurs :</label>
          <div class="controls">

            <!-- this selectd the field to search on -->
            <input type="hidden" name="search_field" value="sous_categorie" />
            ...
            PDb_FormElement::print_element(
              array(
                'type' => 'dropdown',
                'name' => 'value',
                'options' => pdb_dropdown_values('sous_categorie'),
                'class' => 'search-item',
                'value' => filter_input(INPUT_POST, 'value', FILTER_SANITIZE_STRING)
                )
              );
            ?>
            <!-- <input name="submit_button" class="search-form-submit" data-submit="search" type="submit" value="Rechercher"> -->
          </div>
